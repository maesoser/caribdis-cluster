                ...
                //if (c) *c = '\0';
                 ...
                 if(sscanf(buff, "cpu%d %d      %d      %d      %d      %d      %d      %d      %d      %d      %d\n",
                         &cpuid,&val[0],&val[1],&val[2],&val[3],&val[4],&val[5],&val[6],&val[7],&val[8],&val[9])){
                                        ...
                                        cpuarray[cores].last_full = cpuarray[cores].full;
                                        cpuarray[cores].last_idle = cpuarray[cores].idle;
                                        cpuarray[cores].full = val[0]+
                                                val[1]+val[2]+val[3]+val[4]+
                                                val[5]+val[6]+val[7];
                                        cpuarray[cores].idle = val[3] + val[4];
                                         cores = cores + 1;
                                         val[0] = 0;
                                         val[1] = 0;
                                         val[2] = 0;
                                        ...
                                        uint32_t total = cpuarray[cores].full - cpuarray[cores].last_full;
                                        uint32_t idle = cpuarray[cores ].idle - cpuarray[cores].last_idle;
                                        cpuarray[cores].pcnt = (float)((total-idle)/(float)(total))*100.0;
         ...
         while(fgets(buff, sizeof(buff), netinfo)){
                 ...
                 sscanf( buff," %[^:]: %Lu %*u %*u %*u %*u %*u %*u %*u %Lu %*u %*u %*u %*u %*u %*u %*u",ifname, &rx, &tx );
                        if(strstr(ifname,dev)!=NULL){
                                ...
                                if1->last_tx = if1->now_tx;
                                if1->last_rx = if1->now_rx;
                                if1->now_tx = tx;
                                if1->now_rx = rx;
                                if1->rx = (if1->now_tx - if1->last_tx)/1024;
                                if1->tx = (if1->now_rx - if1->last_rx)/1024;
                        }
         }
         if(fclose(netinfo) != 0){
                 exit(-1);
